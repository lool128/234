...
-------------------------------------------------
-- IŞINLANMA FONKSİYONU (Adım adım - anti-teleport bypass)
-------------------------------------------------
local isTeleporting = false  -- aynı anda iki ışınlanmayı engelle

local function teleportTo(pos, onDone)
    ...
    -- Çok yakınsa (< 20 studs) direkt ışınla, adım gerekmez
    if dist < 20 then
    ...
    task.spawn(function()
        -- Adım sayısını mesafeye göre ayarla:
        -- Her adım ~15 studs, min 3 adım max 12 adım
        local STEP_DIST = 15
        local steps     = math.clamp(math.ceil(dist / STEP_DIST), 3, 12)
        local STEP_WAIT = 0.08   -- adımlar arası bekleme (saniye)

        for i = 1, steps do
            -- Karakter hâlâ var mı kontrol et
            local c  = player.Character
            local rp = c and c:FindFirstChild("HumanoidRootPart")
            if not rp then break end

            local t = i / steps
            -- Ease-out: başta hızlı, sonda yavaş (anti-detect)
            local eased = 1 - (1 - t)^2
            local stepPos = current:Lerp(target, eased)
            rp.CFrame = CFrame.new(stepPos)
            task.wait(STEP_WAIT)
        end

        -- Son adım: tam hedefe otur
        local c2  = player.Character
        local rp2 = c2 and c2:FindFirstChild("HumanoidRootPart")
        if rp2 then
            rp2.CFrame = CFrame.new(target)
        end
...
local function InitScannerUI()
    ...
    oreTpBtn.MouseButton1Click:Connect(function()
        ...
        if bestPart then
            oreTpBtn.Text = "↗ Gidiliyor..."
            oreTpBtn.BackgroundColor3 = Color3.fromRGB(80, 60, 160)
            teleportTo(bestPart.Position, function()
    ...
    rarityFrame.Size = UDim2.new(0.94, 0, 0, 196)
    ...
    rarityGrid.CellSize = UDim2.new(0.47, -2, 0, 30)
    rarityGrid.CellPadding = UDim2.new(0.04, 0, 0, 3)
    ...
    for _, r in ipairs(rarities) do
        ...
        espBtn.Size = UDim2.new(0.60, -2, 1, 0)
        ...
        tracerBtn.Size = UDim2.new(0.19, 0, 0.85, 0)
        tracerBtn.Position = UDim2.new(0.61, 0, 0.075, 0)
        ...
        -- Işınlan butonu: sağ %18
        local tpBtn = Instance.new("TextButton", container)
        tpBtn.Size = UDim2.new(0.18, 0, 0.85, 0)
        tpBtn.Position = UDim2.new(0.81, 0, 0.075, 0)
        tpBtn.Text = "⚡"
        ...
        tpBtn.MouseButton1Click:Connect(function()
            ...
            if nearest then
                tpBtn.Text = "↗"
                tpBtn.BackgroundColor3 = Color3.fromRGB(80, 60, 160)
                teleportTo(nearest.Position, function()
                    tpBtn.Text = "✓"
